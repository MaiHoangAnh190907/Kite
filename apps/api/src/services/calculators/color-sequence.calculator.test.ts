...
import { computeColorSequenceMetrics } from './color-sequence.calculator.js';
import type { GameEvent } from '@kite/shared';

describe('computeColorSequenceMetrics', () => {
  it('should return zeros for empty events', () => {
    const result = computeColorSequenceMetrics([]);
  ...
  it('should compute max sequence length', () => {
    ...
    const result = computeColorSequenceMetrics(events);
